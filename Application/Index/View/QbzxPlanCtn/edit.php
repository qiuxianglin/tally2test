		...
		<form action="__ACTION__" method="post">
			...
			<table>
					...
					<td>
					  <select name="ctn_type_code" class="article" style="width: 137px">
						<volist name="contanierlist" id="cl">
							<option value="{$cl['ctn_type_code']}" <?php if($msg['ctn_type_code']==$cl['ctn_type_code']){echo 'selected';}?> >{$cl['ctn_type_code']}</option>
						</volist>
					  </select>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
					   <input type="text" class="article" name="quantity" required="required" value="{$msg['quantity']}"/>
					</td>
				</tr>
					...
					<td>
					   <select name="ctn_master" class="article" style="width: 137px">
						 <volist name="cmlist" id="cm">
							<option value="{$cm['id']}" <?php if($msg['ctn_master']==$cm['id']){echo 'selected';}?> >{$cm['ctn_master']}</option>
						  </volist>
					   </select>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
					   <select name="flflag" class="article" style="width: 137px">
							<option value="F" <?php if($msg['flflag']=='F'){echo 'selected';}?> >整箱</option>
							<option value="L" <?php if($msg['flflag']=='L'){echo 'selected';}?> >拼箱</option>
					   </select>
					</td>
				</tr>
